<template>
  <q-page class="q-pa-sm">
    <!-- <div class="row">
            <h5> {{$t('players.sportsTrail.selectedPlayer')}}: {{igrac}}</h5>
            <q-btn style="padding:22px;margin:10px;color:red" flat round icon="people" @click="izborIgraca()" >
              <q-tooltip>{{$t('players.sportsTrail.browsePlayers')}}</q-tooltip>
            </q-btn>
        </div> -->
    <q-table
      :title="$t('players.sportsTrail.sportInternship')"
      :data="data"
      ...
      :class="[tacnost ? 'gridUkljucenKlasa' : '']"
    ...
    >
      <template
        v-if="igr != $t('players.sportsTrail.selectPlayer') + ' ->'"
        v-slot:top-right="props"
      >
        <q-btn
          v-if="Object.keys(selektovaniIgrac).length != 0"
          @click="otvordeDialog()"
          :label="$t('players.sportsTrail.newRecord')"
        ...
        <!--<q-btn
            ...
            >
              <q-tooltip
                ...
                v-close-popup
              >{{props.inFullscreen ? $t('players.sportsTrail.exitFullscreen') : $t('players.sportsTrail.enterFullscreen')}}</q-tooltip>
            </q-btn> -->
        ...
        >
          <q-tooltip :disable="$q.platform.is.mobile" v-close-popup>{{
            mode === "grid"
              ? $t("players.sportsTrail.list")
              : $t("players.sportsTrail.grid")
          }}</q-tooltip>
        </q-btn>

        <q-btn
          class="dugmeCVS"
          :label="$t('players.sportsTrail.exportCSV')"
          no-caps
          @click="exportTable"
        />
      </template>
      ...
      <template v-slot:body-cell="props">
        <q-td :props="props" v-if="props.value == props.row.action">
          <div class="q-gutter-sm">
            <q-btn
              ...
              icon="delete"
            >
              <q-tooltip> {{ $t("players.sportsTrail.delete") }}</q-tooltip>
            </q-btn>
            <q-btn
              ...
              icon="edit"
            >
              <q-tooltip> {{ $t("players.sportsTrail.changeData") }}</q-tooltip>
            </q-btn>
          </div>
        </q-td>
        ...
        <q-td
          :props="props"
          v-else-if="compare(props.value, props.row.datumPrelaska)"
              ...
              <q-item class="formular ponisti" style="width: 50%">
                <label class="custom-field one datumInput">
                  <q-input
                    ...
                    @blur="
                      spustiPlaceholder('Span-IP0', editedItem.datumPrelaska)
                    "
                    ref="input0"
                    mask="##/##/####"
                  ...
                  <span id="Span-IP3" class="placeholder">{{
                    $t("players.sportsTrail.transitionDate")
                  }}</span>
                </label>
              </q-item>
              <q-item class="formular" style="width: 50%">
                <label class="custom-field one datumInput">
                  <q-input
                    :readonly="readonly"
                    borderless
                    ...
                    @blur="
                      spustiPlaceholder('Span-IP1', editedItem.datumPrestanka)
                    "
                    ref="input1"
                    mask="##/##/####"
                    :rules="[val => valiDate(val) || 'DD/MM/YYYY']"
                  ...
                  <span id="Span-IP1" class="placeholder">{{
                    $t("players.sportsTrail.endDate")
                  }}</span>
                </label>
              </q-item>

              <q-item style="width: 100%; margin-top: 10px">
                <q-item-section>
                  <!-- <q-input dense outlined v-model="editedItem.tipAktivnosti" :label="$t('players.sportsTrail.typeOfActivity')"></q-input> -->
                  <label class="custom-field one">
                    <q-input
                      ...
                      dense
                      placeholder=" "
                      v-model="editedItem.tipAktivnosti"
                      @focus="popniPlaceholder('Span-IP2')"
                      @input="popniPlaceholder('Span-IP2')"
                      @blur="
                        spustiPlaceholder('Span-IP2', editedItem.tipAktivnosti)
                      "
                      ref="input2"
                      :rules="[
                        val =>
                          (val !== null && val !== '') ||
                          $t('players.sportsTrail.enterTypeOfActivity')
                      ]"
                    />
                    <span id="Span-IP2" class="placeholder">{{
                      $t("players.sportsTrail.typeOfActivity")
                    }}</span>
                  </label>
                </q-item-section>
              </q-item>
            ...
            <q-list>
              <q-item class="formular ponisti" style="">
                <q-input
                  ...
                  outlined
                  v-model="editedItem.datumPrelaska"
                  :label="$t('players.sportsTrail.transitionDate') + ' '"
              ...
              <q-item class="formular ponisti" style="">
                <q-input
                  ...
                  outlined
                  v-model="editedItem.datumPrestanka"
                  :label="$t('players.sportsTrail.endDate') + ' '"
                  class="datumInputReadOnly"
                  mask="##/##/####"
                />
              </q-item>
                ...
                <q-item-section>
                  <label class="custom-field one">
                    <input
                      ...
                      placeholder=" "
                      v-model="editedItem.tipAktivnosti"
                      :label="$t('players.sportsTrail.typeOfActivity')"
                    />
...
export default {
      ...
      columns: [
        {
          name: "datumPrelaska",
          required: true,
          label: this.$t("players.sportsTrail.transitionDate"),
          align: "left",
          field: "datumPrelaska",
          sortable: true,
        ...
        },
        {
          name: "datumPrestanka",
          align: "left",
          label: this.$t("players.sportsTrail.endDate"),
          field: "datumPrestanka",
          sortable: true,
          format: val => date.formatDate(val, "DD/MM/YYYY")
        },
        {
          name: "tipAktivnosti",
          align: "left",
          label: this.$t("players.sportsTrail.typeOfActivity"),
          field: "tipAktivnosti",
          sortable: true
        },
        {
          name: "action",
          align: "left",
          label: this.$t("players.sportsTrail.action"),
          field: "action",
          sortable: true
        }
      ],
  ...
  mounted() {
    ...
    this.editedItem.datumPrelaska = new Date()
      .toJSON()
      .slice(0, 10)
      .replace(/-/g, "/");
    this.editedItem.datumPrestanka = new Date()
      .toJSON()
      .slice(0, 10)
      .replace(/-/g, "/");
  },
  methods: {
    ...
    //........................................
    getDataZaID() {
      let linkStr = this.$apiPutanja + "/sportski-staz/igrac/" + this.igrID;
    ...
    postData() {
      debugger;
      let linkStr = this.$apiPutanja + "/sportski-staz";
      var self = this;
      ...
      var dataString = {
        ...
        datumPrestanka: datumVreme2,
        identIgrac: this.igrID,
        identPrimarnaPozicija: null,
        identSekundarnaPozicija: null,
        identTercijalnaPozicija: null,
        identSelekcija: window.$timID,
        tipAktivnosti: this.editedItem.tipAktivnosti
      };
      ...
      this.$axios
        ...
        .then(function(response) {
          ...
          self.getDataZaID();
          self.$q.notify({
            message: self.$t("players.sportsTrail.newRecordAdded"),
            color: "green"
          });
        })
    ...
    putData() {
      debugger;
      let linkStr =
        this.$apiPutanja + "/sportski-staz/" + this.editedItem.ident;
      var self = this;
      let datumVreme1 = this.getDateTime(this.editedItem.datumPrelaska);
      let datumVreme2 = this.getDateTime(this.editedItem.datumPrestanka);
      ...
      this.$axios
        ...
        .then(function(response) {
          ...
          self.getDataZaID();
          self.$q.notify({
            message: self.$t("players.sportsTrail.measuredDataAdded"),
            color: "green"
          });
        })
        .catch(function(response) {
    ...
    //........................................
    deleteData(item) {
      let linkStr = this.$apiPutanja + "/sportski-staz/" + item.ident;
      var self = this;
      this.$axios
        .delete(linkStr, {
          headers: {
        ...
        .then(function(response) {
          ...
          self.getDataZaID();
          self.$q.notify({
            message: self.$t("players.sportsTrail.measuredDataRemoved"),
            color: "green"
          });
        })
        .catch(function(response) {
    ...
    addRow() {
      ...
      if (
        this.editedItem.tipAktivnosti == null ||
        this.editedItem.tipAktivnosti == ""
      )
        sveOK = false;
    ...
    deleteItem(item) {
      ...
      let obj = {
        title: self.$t("players.sportsTrail.warning") + "!",
        message: self.$t("players.sportsTrail.areYouSureDelete"),
        customCloseBtnText: self.$t("players.sportsTrail.no"),
        disableOverlayClick: true,
        useConfirmBtn: true,
        customConfirmBtnText: self.$t("players.sportsTrail.yes"),
    ...
    otvordeDialog() {
      ...
      this.editedItem.datumPrelaska =
        ...
        this.editedItem.datumPrelaska
          ...
          .replace(/-/g, "/")
          .split("/")[0];
      this.editedItem.datumPrestanka = this.editedItem.datumPrelaska;
      console.log(this.editedItem);
      this.new_customer = true;
      let self = this;
      setTimeout(() => {
    ...
    exportTable() {
      ...
      const content = [this.columns.map(col => wrapCsvValue(col.label))]
        ...
        .join("\r\n");
      let self = this;
      const status = exportFile("sportskiStaz.csv", content, "text/csv");

      if (status !== true) {
        this.$q.notify({
          message: self.$t("players.sportsTrail.browserForbids"),
          color: "negative",
          icon: "warning"
        });
      }
