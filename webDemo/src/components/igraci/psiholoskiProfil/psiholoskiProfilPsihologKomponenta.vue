<template>
  <q-page class="q-pa-sm">
      ...
      :loading="loadingUtakmice"
        ...
        >
        <!-- vrati != -->
          <template v-if="igr != $t('players.psychologicalProfile.tests.selectPlayer') + ' ->'" v-slot:top-right="props">
            <q-btn
          v-if="Object.keys(selektovaniIgrac).length != 0" @click="otvordeDialog()" :label="$t('players.psychologicalProfile.tests.newMesurement')" class="q-mr-xs dugmeNewNesto" />
            ...
            >
              <q-tooltip
                :disable="$q.platform.is.mobile"
                v-close-popup
              >{{mode==='grid' ? $t('players.psychologicalProfile.tests.list') : $t('players.psychologicalProfile.tests.grid')}}</q-tooltip>
            </q-btn>
            ...
            <q-td :props="props" v-if="props.value == props.row.action">
              <div class="q-gutter-sm">
                <q-btn
                  ...
                  icon="delete"
                >
                  <q-tooltip>{{ $t("players.psychologicalProfile.tests.delete") }}</q-tooltip>
                </q-btn>
                <q-btn
                  ...
                  icon="edit"
                >
                  <q-tooltip>{{ $t("players.psychologicalProfile.tests.changeData") }}</q-tooltip>
                </q-btn>
              </div>
            </q-td>

            <q-td :props="props" v-else-if="compare(props.value, props.row.datumVreme)" @click="otvoriPreviewDialog(props.row)">
      ...
      <q-card style="width: 1400px; max-width: 100vw; height:100%; overflow: hidden auto; background-image: linear-gradient(to bottom, #f5f8fb, #ffffff); color: #323b62;" flat bordered>
          <!-- <q-card
            style="
              width: 600px;
              max-width: 60vw;
          ...
          > -->
            <q-card-section class="q-pb-none">
              ...
              ></q-btn>
              <div class="text-h6"
                  style="text-align: left; color: #323b62;">
                 ...
                 <table style=" width:100%; padding-top: 3px; border-radiu" cellpadding="20">
                    ...
                    <div style="width: 100%; display: table; padding: 0px">
                      <div style="display: table-cell; padding: 15px;">
                        <div id="strelica">
                          <p class="naslov">{{$t('players.psychologicalProfile.tests.general')}}</p>
                        </div>

                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.unutrasnjaMotivacija" label markers marker-labels :min="0" :max="10" :step="1"  />
                            {{$t('players.psychologicalProfile.tests.internalMotivation')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.spoljnaMotivacija" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.externalMotivation')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.amotivacija" label markers marker-labels :min="0" :max="10" :step="1"  />
                      ...
                      <div style="display: table-cell; padding: 15px 15px 0px 15px;">
                        ...
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.samopouzdanjeUFizickeSposobnosti" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.selfConfidenceInPhisicalAbilities')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.kognitivnaEfikasnost" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.cognitiveEfficiency')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.psiholoskaRezilijentnost" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.psychologicalResilience')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.koncentracija" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.concentration')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.anksioznost" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.anxiety')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.postignucePodPritiskom" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.achivementUnderPressure')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.reakcijaNaGresku" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.reactionToMistake')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.reakcijaNaKritiku" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.reactionToCriticism')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.procenaTudjihEmocija" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.assessmentOfOtherPeoplesEmotions')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                      <div v-if="this.editedIndex > -1">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.emocijalnaInteligencija" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.emotionalIntelligence')}}
                        </div>
                      </div>
                      ...
                      <div style="display: table-cell; padding: 15px;">
                        ...
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.procenaSopstvenihEmocija" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.assessmentOfOnesOwnEmotions')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.upravljanjeSopstvenimEmocijama" label markers marker-labels :min="0" :max="10" :step="1"  />
                         {{$t('players.psychologicalProfile.tests.managingYourOwnEmotions')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.socijalneVestine" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.socialSkills')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly" dense class="slajderKlasa" v-model="editedItem.koriscenjeEmocija" label markers marker-labels :min="0" :max="10" :step="1"  />
                        {{$t('players.psychologicalProfile.tests.usingEmotions')}}
                        </div>
                        <div style="padding-bottom: 15px;">
                         <q-slider :readonly="readonly"  dense class="slajderKlasa" v-model="editedItem.optimizam" label markers marker-labels :min="0" :max="10" :step="1"  />
            ...
            <q-card-section class="q-pt-none">
              <div style="width: 100%; display: table;">
                <div style="display: table-cell; padding: 15px;">
                  ...
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.unutrasnjaMotivacija" label markers marker-labels :min="0" :max="10" :step="1"  />
                      {{$t('players.psychologicalProfile.tests.internalMotivation')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.spoljnaMotivacija" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.externalMotivation')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.amotivacija" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.amotivation')}}
                  </div>
                ...
                <div style="display: table-cell; padding: 15px;">
                  ...
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.samopouzdanjeUFizickeSposobnosti" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.selfConfidenceInPhisicalAbilities')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.kognitivnaEfikasnost" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.cognitiveEfficiency')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.psiholoskaRezilijentnost" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.psychologicalResilience')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.koncentracija" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.concentration')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.anksioznost" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.anxiety')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.postignucePodPritiskom" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.achivementUnderPressure')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.reakcijaNaGresku" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.reactionToCriticism')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.procenaTudjihEmocija" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.assessmentOfOtherPeoplesEmotions')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                <div v-if="this.editedIndex > -1">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.emocijalnaInteligencija" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.emotionalIntelligence')}}
                  </div>
                </div>
                ...
                <div style="display: table-cell; padding: 15px;">
                  ...
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.procenaSopstvenihEmocija" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.assessmentOfOnesOwnEmotions')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.upravljanjeSopstvenimEmocijama" label markers marker-labels :min="0" :max="10" :step="1"  />
                    {{$t('players.psychologicalProfile.tests.managingYourOwnEmotions')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.socijalneVestine" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.socialSkills')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.koriscenjeEmocija" label markers marker-labels :min="0" :max="10" :step="1"  />
                  {{$t('players.psychologicalProfile.tests.usingEmotions')}}
                  </div>
                  <div style="padding-bottom: 15px;">
                    <q-slider dense readonly dense class="slajderKlasa" v-model="editedItem.optimizam" label markers marker-labels :min="0" :max="10" :step="1"  />
                      {{$t('players.psychologicalProfile.tests.optimism')}}
                  </div>
    ...
    postData() {
      ...
      this.$axios
        ...
        .then(function(response) {
          ...
          self.getDataZaID();
          self.$q.notify({
            message: self.$t('players.psychologicalProfile.tests.newMeasurementSuccessfullyAdded'),
            color: "green"
          });
        })
    ...
    putData() {
      ...
      this.$axios
        ...
        .then(function(response) {
          ...
          self.getDataZaID();
          self.$q.notify({
            message: self.$t('players.psychologicalProfile.tests.measurementDataChangedSuccessfully'),
            color: "green"
          });
        })
        .catch(function(response) {
    ...
    deleteData(item) {
      ...
      this.$axios
        ...
        .then(function(response) {
          ...
          self.getDataZaID();
          self.$q.notify({
            message: self.$t('players.psychologicalProfile.tests.measurementDataDeletedSuccessfully'),
            color: "green"
          });
        })
        .catch(function(response) {
