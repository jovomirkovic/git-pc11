<template>
  <q-page class="q-pa-sm">
        ...
        <q-table
          :title="$t('administration.technique.groups.grpTech')"
          :data="data"
        ...
        >
          <template v-slot:top-right="props">
            <q-btn
              @click="otvordeDialog()"
              :label="$t('administration.technique.groups.newGroup')"
            ...
            <q-card-section>
             <q-btn
              ...
              <div class="text-h6"
                ...
                {{$t('administration.technique.groups.elem')}}
              </div>
            </q-card-section>
                  ...
                  <q-item>
                    <q-item-section>
                      ...
                      :readonly="readonly"
                        ...
                        <span id="Span-IP1" class="placeholder">{{$t('administration.technique.groups.grpName')}}</span>
                      </label>
                    </q-item-section>
                  </q-item>
                    ...
                    <q-item-section>
                      <label class="custom-field one">
                        <input
                          ...
                          readonly
                          v-model="editedItem.grupa"
                          :label="$t('administration.technique.groups.grpName')"
                         />
                        <span class="placeholder">{{$t('administration.technique.groups.grpName')}}</span>
                      </label>
                    </q-item-section>
                  </q-item>
    ...
    return {
      ...
      columns: [
        ...
        // },
        {
          name: "grupa",
          align: "left",
          label: this.$t('administration.technique.groups.grp'),
          field: "grupa",
          sortable: true
        },
        {
...
debugger
      this.$axios
        ...
        .then(function(response) {
          ...
          self.getData();
          self.$q.notify({
            message: self.$t('administration.technique.groups.msgPositive'),
          color: "green"
        });
        })
    ...
    putData() {
    debugger
      let linkStr = this.$apiPutanja +  '/baza-tehnike-sifrarnik-grupa/' + this.editedItem.ident
      var self=this;
...
debugger
      this.$axios
        ...
        .then(function(response) {
          ...
          self.getData();
          self.$q.notify({
            message: self.$t('administration.technique.groups.msgChange'),
          color: "green"
        });
        })
        .catch(function(response) {
    ...
    deleteData(item) {
    debugger
      let linkStr = this.$apiPutanja +  '/baza-tehnike-sifrarnik-grupa/' + item.ident
      var self=this;
debugger
      this.$axios
        .delete(linkStr, {
        ...
        .then(function(response) {
          ...
          self.getData();
          self.$q.notify({
            message: self.$t('administration.technique.groups.msgDel'),
          color: "green"
        });
        })
        .catch(function(response) {
    ...
    addRow() {
      ...
      if(sveOK == false) {
        this.$q.notify({
            message: this.$t('administration.technique.groups.notSaved'),
            color: "red"
          });
        this.zacrveniPrazne();
      } else {
    ...
    deleteItem(item) {
      ...
      let obj = {
        ...
        message: this.$t('administration.technique.groups.msgDelConf'),
    ...
    exportTable() {
      ...
      const status = exportFile("tehnika.csv", content, "text/csv");

      if (status !== true) {
        this.$q.notify({
          message: this.$t('administration.technique.groups.msgPrtSc'),
          color: "negative",
          icon: "warning"
        });
      }
