<template>
  <q-page class="q-pa-sm">
        ...
        <q-table
          :title="$t('administration.elementsOfTheGame.elementsOfTheFootballGame')"
          :data="data"
      ...
      :loading="loadingUtakmice"
        ...
        >
          <template v-slot:top-right="props">
            <q-btn
              @click="otvordeDialog()"
              :label="$t('administration.elementsOfTheGame.newElem')"
          ...
          <template v-slot:body-cell="props">
            ...
            <q-td :props="props" v-else-if="props.value == props.row.sifra" @click="otvoriPreviewDialog(props.row)">
                    ...
                    <q-item-section>
                      <label class="custom-field one">
                        <q-input
                          ...
                          @blur="spustiPlaceholder('Span-IP1', editedItem.sifra)"
                          mask="##"
                          ref="input1"
                          :rules="[
                            (val) =>
                              (val !== null && val !== '') ||
                              $t('administration.elementsOfTheGame.pswEnter'),
                          ]"/>
                        <span id="Span-IP1" class="placeholder">{{$t('administration.elementsOfTheGame.psw')}}</span>
                      </label>
                    </q-item-section>
                  </q-item>
                      ...
                      <label class="custom-field one">
                        <q-input
                          ...
                          :rules="[
                            (val) =>
                              (val !== null && val !== '') ||
                              $t('administration.elementsOfTheGame.elemNameEnter'),
                          ]"/>
                        <span id="Span-IP2" class="placeholder">{{$t('administration.elementsOfTheGame.elemName')}}</span>
                      </label>
                    </q-item-section>
                  </q-item>
                    ...
                    <q-item-section>
                      <label class="custom-field one">
                        <q-input
                          ...
                          @blur="spustiPlaceholder('Span-IP3', editedItem.opis)"
                         />
                        <span id="Span-IP3" class="placeholder">{{$t('administration.elementsOfTheGame.elemDesc')}}</span>
                      </label>
                    </q-item-section>
                  </q-item>
                    ...
                    <q-item-section>
                      <label class="custom-field one">
                        <input
                          ...
                          readonly
                          v-model="editedItem.sifra"
                          :label="$t('administration.elementsOfTheGame.psw')"
                        />
                        <span class="placeholder">{{$t('administration.elementsOfTheGame.psw')}}</span>
                      </label>
                    </q-item-section>
                  </q-item>
                    ...
                    <q-item-section>
                      <label class="custom-field one">
                        <input
                          ...
                          readonly
                          v-model="editedItem.naziv"
                          :label="$t('administration.elementsOfTheGame.elemName')"
                          />
                        <span class="placeholder">{{$t('administration.elementsOfTheGame.elemName')}}</span>
                      </label>
                    </q-item-section>
                  </q-item>
                    ...
                    <q-item-section>
                      <label class="custom-field one">
                        <input
                          ...
                          readonly
                          v-model="editedItem.opis"
                          :label="$t('administration.elementsOfTheGame.elemDesc')"
                         />
                        <span class="placeholder">{{$t('administration.elementsOfTheGame.elemDesc')}}</span>
                      </label>
                    </q-item-section>
                  </q-item>
      ...
      columns: [
        {
          name: "sifra",
          align: "left",
          label: this.$t('administration.elementsOfTheGame.psw'),
          field: "sifra",
          sortable: true
        },
        {
          name: "naziv",
          align: "left",
          label: this.$t('administration.elementsOfTheGame.name'),
          field: "naziv",
          sortable: true
        },
        {
...
debugger
      this.$axios
        ...
        .then(function(response) {
          ...
          self.getData();
          self.$q.notify({
            message: self.$t('administration.elementsOfTheGame.msgPositive'),
          color: "green"
        });
        })
    ...
    putData() {
    debugger
      let linkStr = this.$apiPutanja +  '/element-fudbalske-igre-sifrarnik/' + this.editedItem.ident
      var self=this;
      let datetime = this.getDateTime();
...
debugger
      this.$axios
        ...
        .then(function(response) {
          ...
          self.getData();
          self.$q.notify({
            message: self.$t('administration.elementsOfTheGame.msgChange'),
          color: "green"
        });
        })
        .catch(function(response) {
    ...
    deleteData(item) {
    debugger
      let linkStr = this.$apiPutanja +  '/element-fudbalske-igre-sifrarnik/' + item.ident
      var self=this;
debugger
      this.$axios
        .delete(linkStr, {
        ...
        .then(function(response) {
          ...
          self.getData();
          self.$q.notify({
            message: self.$t('administration.elementsOfTheGame.msgDel'),
          color: "green"
        });
        })
        .catch(function(response) {
    ...
    addRow() {
      ...
      if(this.editedItem.sifra == null || this.editedItem.sifra == '')  sveOK = false
      if(this.editedItem.naziv == null || this.editedItem.naziv == '')  sveOK = false

      if(sveOK == false) {
        this.$q.notify({
            message: this.$t('administration.elementsOfTheGame.notSaved'),
            color: "red"
          });
        this.zacrveniPrazne();
      } else {
    ...
    deleteItem(item) {
      ...
      let obj = {
        title: this.$t('administration.elementsOfTheGame.war'),
        message: this.$t('administration.elementsOfTheGame.msgDelConf'),
        customCloseBtnText: this.$t('administration.elementsOfTheGame.txtNo'),
            disableOverlayClick: true,
            useConfirmBtn: true,
        customConfirmBtnText: this.$t('administration.elementsOfTheGame.txtYes'),
    ...
    exportTable() {
      ...
      const status = exportFile("elementi.csv", content, "text/csv");

      if (status !== true) {
        this.$q.notify({
          message: this.$t('administration.elementsOfTheGame.msgPrtSc'),
          color: "negative",
          icon: "warning"
        });
      }
