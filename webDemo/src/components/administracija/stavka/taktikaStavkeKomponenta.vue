  ...
  <q-page class="q-pa-sm">
    ...
    <template v-slot:before>
      ...
      <!-- tabela za grupe -->
        ...
        <q-table
          :title="$t('administration.tactics.items.grpTac')"
          :data="dataGrupe"
      ...
      :loading="loadingUtakmice"
        ...
        >
          ...
          <template >
            <q-input outlined dense debounce="300" v-model="filter" :placeholder="$t('table.search')">
    ...
    <template v-slot:after>
      <q-table
          :title="$t('administration.tactics.items.tacItemsG')"
          :data="data"
          :hide-header="mode === 'grid'"
          :columns="columns"
          row-key="id"
      ...
      :loading="loadingUtakmice"
        ...
        >
          ...
          <template v-slot:top-right="props">
            <q-btn v-if="selectedGrupa.length > 0" @click="otvordeDialog()" :label="$t('administration.tactics.items.new1Item')" class="q-mr-xs dugmeNewNesto" />
          ...
          >
            <q-card-section>
              ...
              <div class="text-h6"
               ...
               <!-- {{this.naslov}} -->
                <!-- {{this.selectedGrupa.naziv}} -->
                {{$t('administration.tactics.items.tacItems')}}
              </div>
            </q-card-section>
            ...
            <q-card-section class="q-pt-none">
              <q-form class="q-gutter-md">
                <q-list>
                  ...
                  <q-item>
                    <q-item-section>
                      <!-- <q-input dense outlined v-model="editedItem.naziv" :label="$t('administration.tactics.items.name')"
                      :rules="[val => val !== null && val !== '' || $t('administration.tactics.items.nameEnter')]"
                      ></q-input>          -->
                      ...
                      :readonly="readonly"
                          ...
                          :rules="[
                            (val) =>
                              (val !== null && val !== '') ||
                              $t('administration.tactics.items.nameEnter'),
                          ]"/>
                        <span id="Span-IP1" class="placeholder">{{$t('administration.tactics.items.name')}}</span>
                      </label>
                    </q-item-section>
                  </q-item>
                  ...
                  <q-item>
                    <q-item-section>
                      <label>Opis</label>
                      <q-editor
                      :readonly="readonly" v-model="editedItem.opis" :label="$t('administration.tactics.items.desc')" min-height="5rem" />
                    </q-item-section>
                  </q-item>

                  <q-item>
                    <q-item-section>
                      <!-- <q-input dense outlined v-model="editedItem.videoZapis" :label="$t('administration.tactics.items.video')"></q-input> -->
                      <label class="custom-field one">
                        <input
                      :readonly="readonly" type="text" placeholder=" "
                        v-model="editedItem.videoZapis"
                          ref="input2"
                        :label="$t('administration.tactics.items.video')"
                        />
                        <span class="placeholder">{{$t('administration.tactics.items.video')}}</span>
                      </label>
                    </q-item-section>
                  </q-item>
                ...
                <q-list>
                    ...
                    <q-item-section>
                      <label class="custom-field one">
                        <input
                          ...
                          placeholder=" "
                          v-model="editedItem.naziv"
                          :label="$t('administration.tactics.items.name')"
                        />
                        <span class="placeholder">{{$t('administration.tactics.items.name')}}</span>
                      </label>
                    </q-item-section>
                  </q-item>
                  ...
                  <q-item>
                      <q-card flat bordered>
                        <q-card-section v-html="editedItem.opis"  style="border: 0.5px solid #323b62; border-radius: 5px;"/>
                      </q-card>
                  </q-item>
...
export default {
  data() {
    return {
      ...
      naslov: "Stavke taktike za grupu: ",
      filter: "",
      filterGrupe: "",
      splitterModel: 28,
      selectedGrupa: [],
      ...
      editedItem: {
        ident: 0,
        bazaTaktikeSifrarnikGrupeIdent: 0,
        naziv: "",
        opis: "",
        sifra: 0,
        videoZapis: ""
      ...
      columns: [
        ...
        // },
        {
          name: "naziv",
          align: "left",
          label: this.$t('administration.tactics.items.name'),
          field: "naziv",
          sortable: true
        },
        {
      ...
      columnsGrupe: [
        {
          name: "grupa",
          required: true,
          label: this.$t('administration.tactics.items.group'),
          align: "left",
          field: "grupa",
          sortable: true
        },
  ...
  methods: {
    ...
    //........................................
    getDataStavke() {
      let linkStr = this.$apiPutanja +  '/baza-taktike-sifrarnik-stavke?identGrupe=' + this.selectedGrupa[0].ident
      //debugger;
        this.$axios
          .get(linkStr,{
                headers: {
    ...
    postData() {
    debugger
      let linkStr = this.$apiPutanja +  '/baza-taktike-sifrarnik-stavke'
      var self=this;
      //let datetime = this.getDateTime();

      var dataString = {
        bazaTaktikeSifrarnikGrupeIdent: this.selectedGrupa[0].ident,
...
debugger
      this.$axios
        ...
        .then(function(response) {
          ...
          self.getDataStavke();
          self.$q.notify({
            message: self.$t('administration.tactics.items.msgPositive'),
          color: "green"
        });
        })
    ...
    putData() {
    debugger
      let linkStr = this.$apiPutanja +  '/baza-taktike-sifrarnik-stavke/' + this.editedItem.ident
      var self=this;
...
debugger
      this.$axios
        ...
        .then(function(response) {
          ...
          self.getDataStavke();
          self.$q.notify({
            message: self.$t('administration.tactics.items.msgChange'),
          color: "green"
        });
        })
        .catch(function(response) {
    ...
    deleteData(item) {
    debugger
      let linkStr = this.$apiPutanja +  '/baza-taktike-sifrarnik-stavke/' + item.ident
      var self=this;
debugger
      this.$axios
        .delete(linkStr, {
        ...
        .then(function(response) {
          ...
          self.getDataStavke();
          self.$q.notify({
            message: self.$t('administration.tactics.items.msgDel'),
          color: "green"
        });
        })
        .catch(function(response) {
    ...
    deleteItem(item) {
      ...
      let obj = {
            ...
            message: this.$t('administration.tactics.items.msgDelConf'),
...
//''''''''''''''''''''''''''''''''''''''''
    ...
    addRow() {
      ...
      if(sveOK == false) {
        this.$q.notify({
            message: this.$t('administration.tactics.items.notSaved'),
            color: "red"
          });
        this.zacrveniPrazne();
      } else {
    ...
    exportTable() {
      ...
      const status = exportFile("taktika-stavke.csv", content, "text/csv");

      if (status !== true) {
        this.$q.notify({
          message: this.$t('administration.tactics.items.msgPrtSc'),
          color: "negative",
          icon: "warning"
        });
      }
