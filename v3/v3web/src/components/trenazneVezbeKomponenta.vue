<template>
  <q-page class="q-pa-sm">
    <q-table
      :title="$t('trainingExercises.trainingExercises')"
      :data="filteredData"
    ...
    >
      ...
      <template v-slot:top-right="props">
        <q-btn
          @click="otvordeDialog()"
          :label="$t('trainingExercises.newExercise')"
      ...
      <template v-slot:body-cell="props">
        ...
        <q-td
          :props="props"
          v-else-if="props.value == props.row.kategorija"
          @click="otvoriPreviewDialog(props.row)"
        >
          <div>
            {{ props.value }}
              ...
              <q-item>
                <q-item-section class="custom-field one">
                  <q-select
                    ...
                    @blur="spustiPlaceholder('Span-SL1', editedItem.kategorija)"
                    :rules="[
                      val =>
                        (val !== null && val !== '') ||
              ...
              <q-item>
                <q-item-section class="custom-field one">
                  <q-select
                    ...
                    use-chips
                    :options="listOptions"
                    v-model="editedItem.identUzrastiList"
                    emit-value
                    map-options
                    ref="select2"
                    :style="{
              ...
              <q-item>
                <q-item-section>
                  <q-select
                    ...
                    :label="$t('trainingExercises.selectCategory')"
                    :options="listOptionsKategorije"
                    v-model="editedItem.kategorija"
                    emit-value
                    map-options
              ...
              <q-item>
                <q-item-section>
                  <q-select
                    ...
                    readonly
                    style="background: #f5f8fb; color: #323b62; height: 45px; border-radius: 5px; margin-top: 10px; padding-left: 14px !important;"
                    :label="$t('trainingExercises.selectAges')"
                    multiple
                    :options="listOptions"
                    v-model="editedItem.identUzrastiList"
                    emit-value
    ...
    filtrirajTV() {
      ...
      this.data.forEach(function(entry) {
        if (self.filter.includes("*")) {
          ...
          if (missCounter == 0) self.filteredData.push(entry);
        } else if (entry.naziv.toLowerCase().includes(self.filter.toLowerCase())) self.filteredData.push(entry);
      });
    },
    popniPlaceholder(id) {
