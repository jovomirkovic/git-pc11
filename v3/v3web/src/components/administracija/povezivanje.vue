/* Dakle, postupak formiranja jednog tima bi bio sledeći: 1. učitamo spisak svih
timova 2. slektujemo tim za koji želimo da pravimo selekciju 3. učitamo spisak
trenera 4. učitam o spisak kondicionih trenera (ili su i oni treneri?) 5.
učitamo spisak igrača 6. izaberemo trenera 7. izaberemeo kondicionog trenera 8.
izabaremeo igrača koga dodajemo u tim 9. učitamo spisak svih pozicija 10.
dodlimo primarnu poziciju igraču 11. dodelimo sekundarnu poziciju igraču 12.
dodelimo tercijalnu poziciju igraču 13. ponovimo to za sve igrače koje dodajemo
u tim 14. SAVE - snimimo igrace u tim, jednog po jednog - povezemo pozicije
igraca u tom timu (jednu po jednu ili sve..?) - povezemo trenera sa timom -
povezemo kondiciong trenera sa timom 15. ende */
<template>
  <q-page class="q-pa-sm">
    <q-splitter
      v-model="splitterModel"
      separator-class="bg-red"
      separator-style="width: 0px; height:80vh"
      style="height: 100%"
    >
      <template v-slot:before>
        <div class="scroll" style="height: 100%">
              ...
              >
                <q-item-section>
                  <q-select
                    ...
                    "
                    outlined
                    :label="$t('administration.connections.team')"
                    option-value="value"
                    option-label="label"
                    :options="listaTimova"
                    map-options
                    v-model="selektovaniTim"
                    v-on:input="promenaTimaBre"
                  />
                </q-item-section>
              </q-item>
            </div>
          ...
          <q-expansion-item
            ...
            header-style="background: #3b5d99; color: white;"
            icon="fas fa-users-cog fa-2x"
            expand-icon-class="text-white"
            @click="
              tabelaJedanZatvorena = !tabelaJedanZatvorena;
              jednaUkljucenaFn();
            "
          >
            <template v-slot:header>
              <span style="width: 100%;">
                <span style="display: inline-block; margin: 9px 0px">{{
                  $t("administration.connections.professionalStaff")
                }}</span>
                <q-input
                  outlined
                  @click.stop
            ...
            <q-table
              ...
              selection="multiple"
              :selected-rows-label="getSelectedStringTim"
              :selected.sync="selectedTim"
              @selection="onSelection1"
              :loading="loadingUtakmice"
              class="mojaTabela my-sticky-header-table tabela1"
              style="
          ...
          <q-expansion-item
            ...
            @click="
              tabelaDvaZatvorena = !tabelaDvaZatvorena;
              jednaUkljucenaFn();
            "
            expand-icon-class="text-white"
          >
            ...
            <template v-slot:header>
              <span style="width: 100%;">
                <span style="display: inline-block; margin: 9px 0px">{{
                  $t("administration.connections.players")
                }}</span>
                <q-input
                  outlined
                  @click.stop
            ...
            <q-table
              ...
              :loading="loadingUtakmice1"
              class="mojaTabela my-sticky-header-table tabela2"
              style="
                background-color: unset;
                box-shadow: unset;
              "
              ...
              <template v-slot:body-cell-action="props1">
                <q-td :props="props1">
                  <div class="q-gutter-sm">
                    <q-btn
                      @click="pozicije(props1.row)"
                      dense
                      color="primary"
                      icon="person"
                    >
                      <q-tooltip>{{
                        $t("administration.connections.positionDef")
                      }}</q-tooltip>
                    </q-btn>
                  </div>
                </q-td>
            ...
            <p style="font-size: initial; font-family: sans-serif; color:red;">{{this.ekipa.imeTima}}</p>
                  ...
                  <q-menu touch-position context-menu>
                    <q-list dense style="min-width: 100px">
                      <q-item clickable v-close-popup>
                        <q-item-section @click="skloniStrucniTim(d, true)">{{
                          $t("administration.connections.delete")
                        }}</q-item-section>
                      </q-item>
                    </q-list>
                  </q-menu>
                  <q-card-section class="col">
                    <div class="text-h col-2" style="height: 50px">
                      {{ d.rolesOpisno[1] }} : {{ d.ime + " " + d.prezime }}
                    </div>
                  </q-card-section>
                </q-card>
              </div>
              <span
                v-for="igrac in selected"
                :key="igrac.id + '@1'"
                class="clickMouse"
                style="
                    width: 110px;
              ...
              >
                <q-menu touch-position context-menu>
                  <q-list dense style="min-width: 100px">
                    <q-item clickable v-close-popup>
                      <q-item-section @click="pozicije(igrac, true)">{{
                        $t("administration.connections.changeData")
                      }}</q-item-section>
                    </q-item>
                    <q-item clickable v-close-popup>
                      <q-item-section @click="brisanje(igrac, true)">{{
                        $t("administration.connections.delete")
                      }}</q-item-section>
                    </q-item>
                  </q-list>
                ...
                <img
                  :src="igrac.fotografija"
                  style="width: 110px; height: 146px; object-fit: cover;"
                />
                <p
                ...
                <p
                  class="backgroundChange flex flex-center"
                  :style="{
                    backgroundColor: [
                      vratiBojuPozicije(igrac.primarnaPozicija.code)
                    ]
                  }"
                  style="height: 75px; font-size: 10pt; margin: 0px; padding: 10px; background-color:#323b62; color: white;"
                >
                  <span>{{
                    igrac.primarnaPozicija != undefined
                      ? igrac.primarnaPozicija.name
                      : ""
                  }}</span>
                </p>
              </span>
        ...
        <q-card-section class="q-pt-none">
          <q-form class="q-gutter-md">
            <q-list>
              <q-item
                class="formular row"
                style="height: 110px; width: 100%; padding: 10px"
              >
                    ...
                    <q-item-section>
                      <label class="custom-field one">
                        <q-input
                          ...
                          @blur="
                            spustiPlaceholder('Span-IP0', editedItem.igrac)
                          "
                        />
                        <span id="Span-IP0" class="placeholder">{{
                          $t("administration.connections.player")
                        }}</span>
                      </label>
                    </q-item-section>
                  </q-item>
                ...
                >
                  <label class="custom-field one datumInput">
                    <q-input
                      ...
                      dense
                      placeholder=" "
                      v-model="editedItem.datumPrelaska"
                      @focus="popniPlaceholder('Span-IP1')"
                      @input="
                        popniPlaceholder('Span-IP1');
                        promenaDatuma(editedItem.id, editedItem.datumPrelaska);
                      "
                      @blur="
                        spustiPlaceholder('Span-IP1', editedItem.datumPrelaska)
                      "
                      ref="input1"
                      mask="##/##/####"
                    ...
                    <span id="Span-IP1" class="placeholder">{{
                      $t("administration.connections.transferDate")
                    }}</span>
                  </label>
                </q-item>
              </q-item>
              ...
              <q-item>
                <q-item-section class="custom-field one">
                  <q-select
                    ...
                    option-value="ident"
                    option-label="name"
                    :options="listaPozicija"
                    map-options
                    ref="select1"
                    @focus="popniPlaceholder('Span-SL1')"
                    @input="popniPlaceholder('Span-SL1')"
                    @blur="
                      spustiPlaceholder('Span-SL1', editedItem.primarnaPozicija)
                    "
                    v-model="editedItem.primarnaPozicija"
                    v-on:input="
                      promenaPrimarnePozicije(
                        editedItem.id,
                        editedItem.primarnaPozicija
                      )
                    "
                    :rules="[
                      val =>
                  ...
                  <span
                    ...
                    >{{ $t("administration.connections.positionPrim1") }}</span
                  >
                </q-item-section>
              </q-item>
                ...
                <q-item-section class="custom-field one">
                  <q-select
                    ...
                    @blur="
                      spustiPlaceholder(
                        'Span-SL2',
                        editedItem.sekundarnaPozicija
                      )
                    "
                    v-model="editedItem.sekundarnaPozicija"
                    v-on:input="
                      promenaSekundarnePozicije(
                        editedItem.id,
                        editedItem.sekundarnaPozicija
                      )
                    "
                  ...
                  <span
                    ...
                    id="Span-SL2"
                    class="placeholder placeholder1"
                    >{{ $t("administration.connections.positionSec1") }}</span
                  >
                </q-item-section>
              </q-item>
              <q-item>
                <q-item-section class="custom-field one">
                  <q-select
                    ...
                    @blur="
                      spustiPlaceholder(
                        'Span-SL3',
                        editedItem.tercijarnaPozicija
                      )
                    "
                    v-model="editedItem.tercijarnaPozicija"
                    v-on:input="
                      promenaTercijarnePozicije(
                        editedItem.id,
                        editedItem.tercijarnaPozicija
                      )
                    "
                  ...
                  <span
                    ...
                    id="Span-SL3"
                    class="placeholder placeholder1"
                    >{{ $t("administration.connections.positionTerc1") }}</span
                  >
                </q-item-section>
              </q-item>
            </q-list>
    ...
    <q-dialog persistent v-model="dialog_za_brisanje_igraca">
      <q-card style="width: 600px; max-width: 60vw;">
          ...
          <q-form class="q-gutter-md">
            <q-list>
                    ...
                    <q-item-section>
                      <label class="custom-field one">
                        <q-input
                          ...
                          @blur="
                            spustiPlaceholder('Span-IP2', editedItem.igrac)
                          "
                        />
              ...
              <q-item tag="label" v-ripple @click="podigni()">
                ...
                <q-item-section>
                  <q-item-label>{{
                    $t("administration.connections.playerLeave")
                  }}</q-item-label>
                </q-item-section>
              </q-item>
              ...
              <q-item
                v-if="izborBrisanja == 'brisiRegularno'"
                class="formular1 ponisti"
                style="width: 35%; margin-top: 10px;"
              >
                <label class="custom-field one datumInput">
                  <q-input
                    ...
                    dense
                    placeholder=" "
                    v-model="editedItem.datumPrestanka"
                    @focus="popniPlaceholder('Span-IP3')"
                    @input="
                      popniPlaceholder('Span-IP3');
                      () => $refs.qDateProxy1.hide();
                    "
                    @blur="
                      spustiPlaceholder('Span-IP3', editedItem.datumPrestanka)
                    "
                    ref="input3"
                    mask="##/##/####"
                    :rules="[val => valiDate(val) || 'DD/MM/YYYY']"
                  ...
                  <span id="Span-IP3" class="placeholder">{{
                    $t("administration.connections.dateTermination")
                  }}</span>
                </label>
              </q-item>
              ...
              <q-item tag="label" v-ripple>
                ...
                <q-item-section>
                  ...
                  <q-item-label caption>{{
                    $t("administration.connections.typeErr")
                  }}</q-item-label>
                </q-item-section>
              </q-item>
            </q-list>
        ...
        <q-card-actions align="center" class="bg-white text-teal ponisti">
          ...
          ></q-btn>
          <q-btn
            :label="$t('administration.connections.accept')"
            @click="idemoNaBrisanje()"
            type="submit"
            style="background-color: #ff4b00; color: white; width: 20%; margin: 10px;"
          ></q-btn>
        </q-card-actions>
  ...
  methods: {
      ...
      this.$axios
        ...
        .then(response => {
          ...
          self.selected.sort((a, b) => {
            if (a.id == self.lastAddedPlayerID) return -1;
            if (
              pomocna232.indexOf(a.primarnaPozicija.code) >
              pomocna232.indexOf(b.primarnaPozicija.code)
            )
              return 1;
            if (
              pomocna232.indexOf(a.primarnaPozicija.code) <
              ...
              return -1;
            if (a.ime + " " + a.prezime > b.ime + " " + b.prezime) return 1;
            if (b.ime + " " + b.prezime > a.ime + " " + a.prezime) return -1;
          ...
          setTimeout(() => {
            self.selectedTim.sort((a, b) =>
              a.id == self.lastAddedStaffID
                ? -1
                : a.ime + " " + a.prezime > b.ime + " " + b.prezime
                ? 1
                : b.ime + " " + b.prezime > a.ime + " " + a.prezime
                ? -1
                : 0
            );
          }, 100);
      ...
      this.$axios
        ...
        .then(response => {
          ...
          self.dataTim.forEach((entry, index) => {
            self.dataTim[index].rolesOpisno = entry.rolesOpisno
              .filter(e => !e.includes("Professional staff"))
              .join(", ");
          });
          console.log(self.dataTim.rolesOpisno);
          console.log("self.dataTim");
    ...
    onSelection({ rows, added, evt }) {
      ...
      if (added == true) {
        this.$q.notify({
          message:
            this.$t("administration.connections.msgPlayerInfoPos") +
            rows[0].igrac,
          color: "black"
        });
    ...
    getSelectedStringTim() {
      return this.selectedTim.length === 0
        ? ""
        : `${this.selectedTim.length} record${
            this.selectedTim.length > 1 ? "s" : ""
          } selected`;
    },
    getSelectedString() {
    ...
    snimiEkipu() {
      ...
      try {
        ...
        self.$q.notify({
          message: self.$t("administration.connections.msgEnterData"),
          color: "red"
        });
        return;
      }
    ...
    dajIgraceZaPut(item, index) {
      debugger;
      if (item.datumPrelaska == undefined) {
        item.datumPrelaska = this.getDateTime(Date.now());
      }
      return {
        id: item.id,
    ...
    brisanje(item, test) {
      ...
      this.editedItem.datumPrestanka =
        this.editedItem.datumPrestanka
          .slice(0, 10)
          .replace(/-/g, "/")
          .split("/")[2] +
    ...
    addRow() {
      ...
      if (
        this.editedItem.primarnaPozicija == undefined ||
        this.editedItem.primarnaPozicija == ""
      )
        sveOK = false;
      if (
        this.editedItem.sekundarnaPozicija == undefined ||
        this.editedItem.sekundarnaPozicija == ""
      )
        sveOK = false;
      if (
        this.editedItem.tercijarnaPozicija == undefined ||
        this.editedItem.tercijarnaPozicija == ""
      )
        sveOK = false;
    ...
    odeIzSelekcije() {
      ...
      let linkStr =
        this.$apiPutanja +
        "/istorija-igraca-kroz-klub/set-datum-prestanka-to-active";
    ...
    jednaUkljucenaFn() {
      ...
      debugger;
      var otvorena = 1;
      if (this.tabelaJedanZatvorena && !this.tabelaDvaZatvorena) otvorena = 1;
